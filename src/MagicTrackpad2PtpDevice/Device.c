...
)
{
	...
	if (NT_SUCCESS(Status)) {
		...
		pDeviceContext->DeviceInfo = AmtPtpGetUsbDeviceConfig(pDeviceContext->DeviceDescriptor);
...
)
{
	...
	pSpiSetStatus->Status = Enabled ? 1 : 0;
...
End:
	...
	if (NT_SUCCESS(Status))
	{
		Status = AmtPtpSpiStartInternalReportReadLoop(
			Device
		);
	}
#endif
